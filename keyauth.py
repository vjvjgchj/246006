...
def _allow_unsigned_fallback():
    return os.getenv("KEYAUTH_ALLOW_UNSIGNED_FALLBACK", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    def __do_request(self, post_data):
        try:
            ...
            if not signature or not timestamp:
                if _allow_unsigned_fallback():
                    print("Missing headers for signature verification. Falling back to unsigned response because KEYAUTH_ALLOW_UNSIGNED_FALLBACK is enabled.")
                    return response.text
                print("Missing headers for signature verification. Strict signature verification is enabled, request rejected.")
                time.sleep(3)
                os._exit(1)
