...
function Assert-UnderPath($PathValue, $ParentPath) {
    ...
    if (-not ($full.StartsWith($parent + "\", [System.StringComparison]::OrdinalIgnoreCase))) {
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    ...
    $OutputDir = Join-Path $DistRoot "neko_release_clean_$stamp"
}
...
if ($IncludeLogitechInstaller) {
    Copy-FileIfExists (Join-Path $ProjectRoot "罗技驱动2021.3.exe") $OutputDir | Out-Null
}
...
这是由 `8_make_clean_release.bat` 生成的干净发布包。

## 已保留

- QML 面板入口和必要 Python 文件
- `backend/` 与 `qml/`
- 正式推理核心 `runtime/TRT_ZeroCopy_Pipeline.exe`
- 正式配置 `runtime/config.txt`
- TensorRT/CUDA 运行 DLL
- `runtime/models/*.engine`
- 根目录 `models/*.engine` 模型库
- VC++ x64 运行库安装器
...
- `.bak` 备份文件
- 默认不包含 `.onnx` 源模型
- 默认不包含完整 ONNX -> TensorRT 编译器

## 使用方式

1. 双击 `1_install_vcredist.bat` 安装 VC++ 运行库。
2. 如需使用面板，先安装 Python 依赖：`5_install_panel_python_deps.bat`。
3. 双击 `6_run_qml_panel.vbs` 启动面板。
...
Write-Info "Clean release ready: $OutputDir"
