...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    ...
    $OutputDir = Join-Path $DistRoot "neko_release_web_$stamp"
}
...
Copy-DirFiltered `
    ...
    -IncludeFile { param($File) $File.Extension -ieq ".py" -and $File.Name -ine "qml_bridge.py" }
...
This package is generated by `8_make_clean_release.bat` and uses the Web panel only.

## Included
- Web panel entry: `6_run_web_panel.vbs`
- Web panel controller: `run_web_panel_hidden.pyw` and `backend/`
...
## Removed
- QML panel entry and QML resources
- `.codex/`
- `branches/`
- `simulators/`
- `firmware/`
...
3. Double-click `6_run_web_panel.vbs` to start the Web panel.
...
Write-Info "Web-only clean release ready: $OutputDir"
